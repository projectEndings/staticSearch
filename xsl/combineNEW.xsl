...
<xsl:stylesheet
    ...
    <xsl:template match="/">
        ...
        <xsl:call-template name="combineTitles"/>
        <xsl:call-template name="combineFilters"/>
    ...
    <xsl:template name="combineTitles">
        <xsl:message>Combining Titles</xsl:message>
        <xsl:variable name="path" select="$jsonDirPath || 'titles'"/>
        <xsl:variable name="collectionPath" select="$path || '?select=*.json'"/>
        <xsl:variable name="collection" select="uri-collection($collectionPath)"/>
        <xsl:variable name="result" as="element(map:map)">
            <map xmlns="http://www.w3.org/2005/xpath-functions">
                <xsl:for-each select="$collection">
                    <xsl:variable name="jsonDoc" select="unparsed-text(.) => json-to-xml()"/>
                    <xsl:sequence select="$jsonDoc//map:array"/>
                </xsl:for-each>
            </map>
        </xsl:variable>
    ...
    <xsl:template name="combineFilters">
        <xsl:message>Combining Filters</xsl:message>
        <xsl:variable name="path" select="$jsonDirPath ||'filters'"/>
        ...
        <xsl:message><xsl:value-of select="$collectionPath"/></xsl:message>
        <!--And now group each collection via its sub directories-->
        <xsl:for-each-group select="$collection" group-by="hcmc:trimPath(.,2)">
     
            <xsl:variable name="filterTypeDir"
                select="current-grouping-key()"/>
            
            <xsl:variable name="filterTypeName" 
                select="tokenize($filterTypeDir,'/')[last()]"/>
            
            <xsl:for-each-group select="current-group()" group-by="hcmc:trimPath(.,1)">
                
                <xsl:variable name="filterNameDir" 
                    select="current-grouping-key()"/>
                
                <xsl:variable name="filterId"
                    select="'ss' 
                    || upper-case(substring($filterTypeName,1,1)) 
                    || substring($filterTypeName, 2) 
                    || position()"/>
                
                
                <xsl:variable name="filterName" 
                    select="unparsed-text($filterNameDir || '/NAME')" 
                    as="xs:string"/>
                
                <xsl:variable name="filterCollection"
                    select="uri-collection($filterNameDir || '?select=*.json')"/>
                
                <xsl:variable name="filterJSONs"
                    select="for $filter in $filterCollection
                            return unparsed-text($filter)"/>
                
                <xsl:variable name="filterXml"
                    select="for $filter in $filterJSONs
                    return json-to-xml($filter)"/>
                
      
       
                <xsl:variable name="result" as="element(map:map)">
                    <map xmlns="http://www.w3.org/2005/xpath-functions">
                        <string key="filterId"><xsl:value-of select="$filterId"/></string>
                        <string key="filterName"><xsl:value-of select="$filterName"/></string>
                        <xsl:choose>
                            <xsl:when test="$filterTypeName = 'desc'">
                                <xsl:for-each-group select="$filterXml"
                                    group-by="descendant::map:array[@key='values']/map:string">
                                    <map key="{$filterId}_{position()}">
                                        <string key="name"><xsl:value-of select="current-grouping-key()"/></string>
                                        <array key="docs">
                                            <xsl:for-each-group select="current-group()" group-by="descendant::map:string[@key='docUri']">
                            ...
                            <xsl:when test="$filterTypeName = 'bool'">
                                <xsl:for-each-group select="$filterXml"
                                    group-by="descendant::map:array[@key='values']/map:string">
                            ...
                            <xsl:when test="$filterTypeName = 'date'">
                                <map key="docs">
                                    <xsl:for-each-group select="$filterXml" group-by="descendant::map:string[@key='docUri']">
                                        <array key="{current-grouping-key()}">
                                            <xsl:for-each select="current-group()/descendant::map:array[@key='values']/map:string">
                            ...
                            <xsl:when test="$filterTypeName = 'num'">
                                <map key="docs">
                                    <xsl:for-each-group select="$filterXml"
                                        group-by="descendant::map:string[@key='docUri']">
                                        <array key="{current-grouping-key()}">
                                            <xsl:for-each select="current-group()/descendant::map:array[@key='values']/map:string">
                                                <xsl:if test=". castable as xs:decimal">
                                                    <string><xsl:value-of select="xs:decimal(.)"/></string>
                                                </xsl:if>
                                            </xsl:for-each>
                ...
                <xsl:result-document
                    href="{$outDir || '/filters/' || $filterId || $versionString || '.json'}" 
                    method="text">
                    <xsl:value-of select="xml-to-json($result)"/>
                </xsl:result-document>
            </xsl:for-each-group>
    ...
    <xsl:function name="hcmc:trimPath">
        <xsl:param name="uri"/>
        <xsl:param name="num"/>
        <xsl:value-of select="string-join(tokenize($uri,'/')[position() = (1 to last() - $num)],'/')"/>
