...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:stem" type="function">h:stem</xd:ref> is the core function that
  ...
  <xsl:function name="h:stem" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="R" select="h:getR1AndR2($preflight)"/>
        <xsl:variable name="step0" select="h:step0($preflight)"/>
        <xsl:variable name="step1" select="h:step1($step0, $R[3])"/>
        <xsl:choose>
          <xsl:when test="$step1 = $step1aExceptions">
            <xsl:value-of select="$step1"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:variable name="step2" select="h:step2($step1, $R[3])"/>
            <xsl:variable name="step3" select="h:step3($step2, $R[3], $R[4])"/>
            <xsl:variable name="step4" select="h:step4($step3, $R[4])"/>
            <xsl:variable name="step5" select="h:step5($step4, $R[3], $R[4])"/>
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:preflight" type="function">h:preflight</xd:ref> does a couple of simple
      replacements that need to precede the actual stemming process.
    </xd:desc>
    <xd:param name="token">Input token string</xd:param>
    <xd:result>The treated version of the token.</xd:result>
  </xd:doc>
  <xsl:function name="h:preflight" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
    <xsl:value-of select="replace(replace(replace($token, '^''', ''), '^y', 'Y'), concat('(', $vowel, ')y'), '$1Y')"/>
  </xsl:function>
  
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:step0" type="function">h:step0</xd:ref> trims plural/possessive
      type suffixes from the end,
    </xd:desc>
    <xd:param name="token">Input token string</xd:param>
    <xd:result>The treated version of the token.</xd:result>
  </xd:doc>
  <xsl:function name="h:step0" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
    <xsl:value-of select="replace($token, '''(s('')?)?$', '')"/>
  </xsl:function>
  
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:step1" type="function">h:step1</xd:ref> performs three replacements
  ...
  <xsl:function name="h:step1" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
...
<!--  Step 1b on the face of it looks like it could be handled by
            ...
            <xsl:choose>
              ...
              <xsl:when test="matches($step1a, $step1b2Rex)">
                ...
                <xsl:value-of select="if (matches($step1b2a, '((at)|(bl)|(iz))$')) then concat($step1b2a, 'e') 
                  ...
                  else if (h:wordIsShort($step1b2a, $R1)) then concat($step1b2a, 'e')
                  else $step1b2a"/>
              </xsl:when>
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:step2" type="function">h:step2</xd:ref> consists
  ...
  <xsl:function name="h:step2" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
    <xsl:param name="R1" as="xs:integer"/>
...
<!--    Once we have a match, we break irrespective of whether we did a replace.        -->
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:step3" type="function">h:step3</xd:ref> consists
      of a sequence of items to be evaluated against the input token; if a match
      occurs, then a) a replacement operation is done ONLY IF the match is in
  ...
  <xsl:function name="h:step3" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
    <xsl:param name="R1" as="xs:integer"/>
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:step4" type="function">h:step4</xd:ref> consists
      of a sequence of items to be evaluated against the input token; if a match
  ...
  <xsl:function name="h:step4" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
    <xsl:param name="R2" as="xs:integer"/>
    <xsl:variable name="resultToken" as="xs:string">
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:step5" type="function">h:step5</xd:ref> consists
  ...
  <xsl:function name="h:step5" as="xs:string">
    <xsl:param name="token" as="xs:string"/>
    <xsl:param name="R1" as="xs:integer"/>
    <xsl:param name="R2" as="xs:integer"/>
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:getR1AndR2" type="function">h:getR1AndR2</xd:ref>
  ...
  <xsl:function name="h:getR1AndR2" as="item()+">
    <xsl:param name="token" as="xs:string"/>
  ...
  <xsl:function name="h:wordIsShort" as="xs:boolean">
    <xsl:param name="token" as="xs:string"/>
    <xsl:param name="R1" as="xs:integer"/>
...
<!--    The second regex matches at the beginning, and we just need to know if it's only a
  ...
  <xd:doc scope="component">
    <xd:desc><xd:ref name="h:runTests" type="function">h:runTests</xd:ref>
      feeds all of the test data into the h:stem function and checks the 
  ...
  <xsl:function name="h:runTests" as="item()+">
    <!--<xsl:iterate select="$porterTestData">
      <xsl:on-completion select="(true(), '')"/>
      <xsl:variable name="result" select="h:stem(substring-before(., ':'))"/>
      <xsl:choose>
    ...
    <xsl:iterate select="$fullTokenSet">
      ...
      <xsl:variable name="result" select="h:stem(.)"/>
      <xsl:choose>
